#!/usr/bin/env python3
# scripts/base/code_generator.py - ENDGÜLTIGE KORREKTUR für alle Test-Typen
...
def generate_test_function(test: dict, test_type: str = "latency") -> Tuple[str, str]:
    ...
    # Für Load/Store-Tests (Priorität 1) - aber mit geringeren Iterationen
    if is_load_store:
...
def generate_throughput_loadstore_function(func_name: str, test: dict, instruction_block: str,
    ...
    uint32_t s0_val = (uint32_t)safe_buffer;
...
"""
    ...
    inputs = """[t0_val] "r"(t0_val), [t1_val] "r"(t1_val), [t2_val] "r"(t2_val),
            [a2_val] "r"(a2_val), [base_ptr] "r"(base_ptr),
...
def generate_loadstore_test_function(func_name: str, test: dict, instruction_block: str,
    ...
    # Stelle sicher, dass a3 (Base-Pointer) immer dabei ist
    used_regs.add('a3')
    ...
    # Basis-Pointer - ALLE zeigen auf safe_buffer
    reg_init_code = [
        '    uint32_t base_ptr = (uint32_t)safe_buffer;'
    ]
    
    # NUR die tatsächlich verwendeten Register initialisieren
    if 's0' in used_regs:
    ...
    # Load-Register-Code - NUR die verwendeten Register laden
    load_regs_code = ['            "mv a3, %[base_ptr]\\n"']
    
    if 's0' in used_regs:
        load_regs_code.append('            "mv s0, %[s0_val]\\n"')
    if 's1' in used_regs:
        load_regs_code.append('            "mv s1, %[s1_val]\\n"')
    if 's2' in used_regs:
        load_regs_code.append('            "mv s2, %[s2_val]\\n"')
    
    load_regs_str = ''.join(load_regs_code)
    
    # Input-Liste - NUR die verwendeten Register
    input_list = ['[base_ptr] "r"(base_ptr)']
    
    if 's0' in used_regs:
        input_list.append('[s0_val] "r"(s0_val)')
    if 's1' in used_regs:
        input_list.append('[s1_val] "r"(s1_val)')
    if 's2' in used_regs:
        input_list.append('[s2_val] "r"(s2_val)')
...
test_result_t {func_name}(void) {{
    ...
    // Array für alle Einzelmessungen - KLEINER für Load/Store
    uint32_t measurements[{iterations}];
    {buffer_decl}
    
    // =======================================================
    // REGISTER-INITIALISIERUNG - NUR verwendete Register
    // =======================================================
...
def generate_latency_test_function(func_name: str, test: dict, instruction_block: str,
    ...
    if len(test["instructions"]) > 10:
        important_regs = {'t0', 't1', 't2', 'a2', 'a3', 'a4', 'a5', 's0'}
    ...
    # Dummy-Buffer
    base_ptr_init = '    uint32_t base_ptr = 0x12345678;'
    
    # Register-Initialisierung
    reg_init_code = [base_ptr_init]
    ...
    for reg in sorted(used_regs):
        if reg != 'a3' and reg in reg_values:
...
def generate_throughput_test_function(func_name: str, test: dict, instruction_block: str,
    ...
    clobber_list = ['"t0"', '"t1"', '"t2"', '"a2"', '"a3"', '"a4"', '"a5"', '"memory"']
    clobber_str = ', '.join(clobber_list)
